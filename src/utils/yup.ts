...
const DEFAULT_POSITIVE_NUMBER_ERROR =
  "Este campo debe ser un valor numérico positivo";
...
const numericString = (options?: NumericStringOptions) => {
  ...
  if (options?.minLength != null) {
    schema = schema.min(
      options.minLength,
      `El campo debe tener al menos ${options.minLength} caracteres`,
    );
  }
  if (options?.maxLength != null) {
    schema = schema.max(
      options.maxLength,
      `La longitud máxima permitida es de ${options.maxLength} caracteres`,
    );
  }
  return schema;
};
